...
> renvoie le nombre de bootstraps joints, `0` si la liste est vide),
...
> `/p2p/<peerid>`, ou borne de `MAX_BOOTSTRAPS` atteinte → `InvalidInput`,
> pas `Network` : le noyau le reporte en `CoreError::Network`, remappé
> explicitement côté FFI), `mdns_enabled() -> bool` (sync — la découverte
> mDNS locale est-elle active ? défaut vrai) et `set_mdns(enabled)` (sync —
> persiste le choix ; **n'a d'effet qu'au prochain démarrage**, le socket
> multicast n'étant ouvert/fermé qu'à la construction du swarm). Purement
> additif.
...
| `bootstrap() -> u32` | **async** | compose vers tous les bootstraps connus (best-effort), peuple la table de routage ; `0` si liste vide |
...
| `add_bootstrap(multiaddr) -> ()` | **async** | ajoute un bootstrap persisté ; sans `/p2p/<peerid>` ou borne atteinte → `InvalidInput` |
| `mdns_enabled() -> bool` | sync | la découverte mDNS locale est-elle active ? (défaut vrai) |
